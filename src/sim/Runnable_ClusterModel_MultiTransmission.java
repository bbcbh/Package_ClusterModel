...
public class Runnable_ClusterModel_MultiTransmission extends Abstract_Runnable_ClusterModel_Transmission {
	...
	protected HashMap<Integer, int[][]> map_current_infection_stage; // Key=PID,V=int[INF_ID][SITE]{infection_stage}
	...
	protected static final int STAGE_ID_DEATH = Integer.MAX_VALUE;
	...
	public Runnable_ClusterModel_MultiTransmission(long cMap_seed, long sim_seed, int[] pop_composition,
		...
		map_current_infection_stage = new HashMap<>();
		...
		try {
			...
			for (int eI : EXPORT_INDEX_ARRAY) {
				...
				case EXPORT_MAP_DEATH:
					...
					switch (eI) {
					case EXPORT_MAP_INF_STAGE:
						map = map_current_infection_stage;
						break;
					case EXPORT_MAP_INF_SWICH:
	...
	public void importRunnableTransmission(int time_pt) throws IOException, ClassNotFoundException {
		...
		for (int eI : EXPORT_INDEX_ARRAY) {
			...
			while ((line = reader.readLine()) != null) {
				...
				case EXPORT_MAP_DEATH:
					...
					switch (eI) {
					case EXPORT_MAP_INF_STAGE:
						tarMap = map_current_infection_stage;
						break;
					case EXPORT_MAP_INF_SWICH:
					...
					if (tarMap != null) {
						tarMap.put(key, (int[][]) util.PropValUtils.propStrToObject(lineSp[1], int[][].class));
					}
					break;
				case EXPORT_SCHEDULE_TESTING:
		...
		for (int pid : map_current_infection_stage.keySet()) {
			int[][] ent = map_current_infection_stage.get(pid);
	...
	public void refreshField(int fieldId, int currentTime, boolean clearAll, String orginal_field) {
					...
					if (switch_prob > 0) {
						if (param.length == 1 || Double.isNaN(param[1])) {
							...
							if (param[1] < 0) {
								...
								// Gamma Distribution
								dist_stage_period[inf_id][site][stage_id_to] = generateGammaDistribution(stage_dist,
										param);
							}
						}
					}
		...
		case RUNNABLE_FIELD_TRANSMISSION_TESTING_RATE_BY_RISK_CATEGORIES:
			// Convert to new test rate if testing already began
			if (test_rate_index_map.size() > 0) {
				HashMap<String, ArrayList<Integer>> existMap = new HashMap<>();
				int testPopSize = 0;
				for (int pid : test_rate_index_map.keySet()) {
					if (isValidPerson(pid, new int[] { currentTime })) {
						HashMap<Integer, Integer> test_pt = test_rate_index_map.get(pid);
						for (Integer testType : test_pt.keySet()) {
							// Format: testType, testPt
							String key = String.format("%d,%d", testType, test_pt.get(testType));
							ArrayList<Integer> pidGrp = existMap.get(key);
							if (pidGrp == null) {
								pidGrp = new ArrayList<>();
								existMap.put(key, pidGrp);
							}
							pidGrp.add(pid);
				...
				RandomGenerator rng_test = new MersenneTwisterRandomGenerator(sIM_SEED);
	...
	public void initialse() {
		...
		map_current_infection_stage.clear();
		...
		for (int currentTime = startTime; currentTime < startTime + nUM_TIME_STEPS_PER_SNAP * nUM_SNAP
						...
						for (int site_id = 0; site_id < NUM_SITE; site_id++) {
							if (state_change_today.get(inf_id).get(site_id) != null
								...
								for (Integer pid : state_change_pids) {
									...
									// Adjusted removal time
									if (currentTime >= exitPopAt(pid)) {
										ArrayList<Integer> rm_today = pid_remove_at.get(exitPopAt(pid));
										if (rm_today == null) {
											rm_today = new ArrayList<>();
											pid_remove_at.put(currentTime, rm_today);
										}
										int pt = Collections.binarySearch(rm_today, pid);
										if (pt < 0) {
											rm_today.add(~pt, pid);
				...
				for (Integer pid_inf_src : currenty_infectious_ent) {
					if (cMap.containsVertex(pid_inf_src)) {
						...
						for (Integer[] e : edges) {
							...
							int g_t = currentTime < exitPopAt(pid_inf_tar) ? getPersonGrp(pid_inf_tar) : -1;
									...
									for (int s = 0; s < 2; s++) { // if s==0 p_1=site_1, else p_1=site_2
										if ((hasActed[a] & 1 << s) != 0) {
											...
											if (src_site == infected_site_id) {
												...
												int[][] tar_infection_stages = map_current_infection_stage
														.get(pid_inf_tar);

												if (isValidInfectionTargetSite(inf_id, tar_site,
														tar_infection_stages)) {

													double transProb = getTransmissionProb(currentTime, inf_id,
															pid_inf_src, pid_inf_tar,
													...
													if (transProb > 0) {
														if (RNG.nextDouble() < transProb) {
															...
															if (tar_infection_stages == null) {
																...
																map_current_infection_stage.put(pid_inf_tar,
																		tar_infection_stages);
															} else {
															...
															updateInfectStageChangeSchedule(pid_inf_tar, inf_id,
																	tar_site, currentTime + 1,
							...
							} // End of checking expired partnership if (g_s != -1 && g_s != -1) {
			...
			if (snap_index == 0) {
				...
				if ((simSetting & 1 << Simulation_ClusterModelTransmission.SIM_SETTING_KEY_GEN_PREVAL_FILE) != 0) {
					...
					for (Integer pid : map_current_infection_stage.keySet()) {
	...
	protected void handleRemovePerson(Integer pid) {
		int[][] stat = map_current_infection_stage.get(pid);
		if (stat != null) {
			...
			map_current_infection_stage.put(pid, stat);
		}
				...
				for (int siteId = 0; siteId < NUM_SITE && !applyTreatment; siteId++) {
					if ((siteIncl & 1 << siteId) != 0) {
						...
						if (test_properties != null) {
							inf_stage = map_current_infection_stage.get(pid);
			...
			for (int i = 0; i < testRateDefs.length; i++) {
				...
				if (testRateDef[FIELD_TESTING_RATE_BY_RISK_CATEGORIES_TEST_RATE_PARAM_START] == -1) {
					...
					if ((gIncl & (1 << genderType)) != 0 && (iIncl & treatment_inf_incl) != 0) {
						...
						int retest_opt_start = FIELD_TESTING_RATE_BY_RISK_CATEGORIES_TEST_RATE_PARAM_START + 2;
						int retest_opt_length = (testRateDef.length - retest_opt_start) / 2;

						double prob = RNG.nextDouble();
						int pt = Arrays.binarySearch(testRateDef, retest_opt_start,
								retest_opt_start + retest_opt_length + 1, prob);
						if (pt < 0) {
							pt = ~pt;
						}
						pt += retest_opt_length;

						if ((pt + 1) < testRateDef.length && !Double.isInfinite(testRateDef[pt])) {
							int nextRetestAfter = (int) (testRateDef[pt]
									+ RNG.nextInt((int) testRateDef[pt + 1] - (int) testRateDef[pt]));

							int nextRetestDate = currentTime + nextRetestAfter;

							if (nextRetestDate < exitPopAt(pid)) {
								ArrayList<int[]> day_sch = schedule_testing.get(nextRetestDate);

								if (day_sch == null) {
									day_sch = new ArrayList<>();
									schedule_testing.put(nextRetestDate, day_sch);
								}
								int[] test_pair = new int[] { pid, iIncl, sIncl };
	...
	protected void scheduleNextTestByDefNum(Integer personId, int testDefNum, int lastTestTime, int mustTestBefore) {
		...
		if (past_test_pt == null || past_test_pt.get(testDefNum) == null) {
			int pt;
			...
			test_pt = test_opt_length + pt;
			past_test_pt.put(testDefNum, test_pt);
		} else {
			test_pt = past_test_pt.get(testDefNum);
		}

		if ((test_pt + 1) < testRateDef.length && !Double.isInfinite(testRateDef[test_pt])) {
			int nextTestAfter = (int) (testRateDef[test_pt]
					+ RNG.nextInt((int) testRateDef[test_pt + 1] - (int) testRateDef[test_pt]));
	...
	public void allocateSeedInfection(int[][] num_infected, int time) {
		...
		if (bASE_CONTACT_MAP != null) {
			...
			cMapLoop: for (int[] inf_setting : num_infected) {
				...
				while (pt < inf_setting.length) {
					if (inf_setting[pt] < 0) {
						if (bASE_CONTACT_MAP == null) {
							initaliseCMap(cMap, null, -1, time, new HashMap<>());
						} else {
		...
		if (bASE_CONTACT_MAP == null && cMap != null) {
			all_vertex = cMap.vertexSet().toArray(new Integer[cMap.vertexSet().size()]);
		} else {
			all_vertex = getCurrentPopulationPId(time);
		}
		...
		for (int[] inf_setting : num_infected) {
			...
			if (inf_id < NUM_INF) {
				int pt = 0;
				while (pt < inf_setting.length) {
					boolean inPartnershipOnly = inf_setting[pt] < 0;
					int includeIndex = Math.abs(inf_setting[pt]);
					pt++;
					...
					if (validInfectiousStages.size() > 0) {
						...
						for (Integer pid : all_vertex) {
							...
							if (inPartnershipOnly && cMap != null) {
				...
				for (Integer pid : all_vertex) {
					...
					if ((gender_inc & 1 << candidate_gender) != 0) {
						int[][] candidate_inf_state = map_current_infection_stage.get(pid);
	...
	public int addInfectious(Integer infectedPId, int infectionId, int site_id, int stage_id, int infectious_time,
		...
		int[][] current_stage_arr = map_current_infection_stage.get(infectedPId);
		if (current_stage_arr == null) {
			...
			map_current_infection_stage.put(infectedPId, current_stage_arr);
		}
	...
	protected int updateInfectionStage(Integer pid, int infection_id, int site_id, int current_infection_stage,
		...
		if (current_stage_arr[infection_id][site_id] != STAGE_ID_DEATH) {
			if (current_infection_stage == STAGE_ID_JUST_INFECTED) {
						...
						if ((riskGrpInc & 1 << riskGrp) != 0) {
							...
							if (FIELD_STAGE_PERIOD_INSTANT_STAGE_OPTIONS_START + 1 < options.length) {
								...
								int opPt = Arrays.binarySearch(options,
										FIELD_STAGE_PERIOD_INSTANT_STAGE_OPTIONS_START + numPossibleStage,
										options.length, RNG.nextDouble());
								if (opPt < 0) {
									opPt = ~opPt;
								}
				...
				int incub_period = (int) dist_stage_period[infection_id][site_id][current_infection_stage].sample();
				infect_switch_time = addInfectious(pid, infection_id, site_id, current_infection_stage, current_time,
						incub_period);

				if (current_stage_arr[infection_id][site_id] != current_infection_stage) {
					// Overwrite
					current_infection_stage = current_stage_arr[infection_id][site_id];
					infect_switch_time = infection_state_switch[infection_id][site_id];
				}
