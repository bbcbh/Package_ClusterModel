...
public class Util_Analysis_Compare_Cumulative {
	...
	final String pWriMapK_key_format = "%s_%s"; /// pattern index,time_to_match
	...
	public void generateAnalysisCSV() throws IOException {
		...
		HashMap<String, String[]> base_entry_map = generateEntryMapFromDirectory(srcDir, compareFilePrefix, header);

		System.out.printf("Reading results from %s completed. # entries in total = %d.\n", srcDir.getAbsolutePath(),
				base_entry_map.size());

		HashMap<String, float[]> base_entry_diff = generateEntryDiffMap(base_entry_map);
		...
		for (int pIndex = 0; pIndex < compareFilePrefix.length; pIndex++) {
			for (int t = 1; t < time_to_match.length; t++) {
				...
				String pWriMapKey = String.format(pWriMapK_key_format, Integer.toString(pIndex), time);
		...
		for (File compDir : compareDirs) {
			HashMap<String, String[]> comp_entry_map = generateEntryMapFromDirectory(compDir, compareFilePrefix, null);
			HashMap<String, float[]> comp_diff = generateEntryDiffMap(comp_entry_map);
			HashMap<String, String[]> linked_entry_map = generateLinkedEntryMap(compDir);

			for (Entry<String, float[]> diff_map_ent : comp_diff.entrySet()) {
				...
				if (b_val == null || val == null) {
					...
					String pWriMapKey = String.format(pWriMapK_key_format, diff_key_split[0], diff_key_split[diff_key_split.length-1]);
		...
		System.out.printf("Compare results for all completed.\n");
		...
		for (Entry<String, String[]> entrySet : entry_map.entrySet()) {
			...
			if (key_split[key_split.length - 1].equals(time_to_match[0])) {
				...
				for (int t = 1; t < time_to_match.length; t++) {
					...
					String diffKey = String.format(linesMap_key_format, key_split[0], key_split[1], key_split[2],time_to_match[t]);
